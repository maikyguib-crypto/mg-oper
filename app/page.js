...
export default function Home() {
  ...
  function memberQuantity(taskId, memberId) {
    return logs
      .filter(l => l.task_id === taskId && l.member_id === memberId)
  ...
  async function startTask(task) {
    ...
    const { error } = await db.from('tasks').update({
      ...
      production_started_at: task.production_started_at || now
    }).eq('id', task.id)
  ...
  async function changeMemberProgress(task, member, amount) {
    setMsg('')

    const currentTotal = Number(task.quantity_done || 0)
    const target = Number(task.quantity_target || task.goal || 0)
    const currentMember = memberQuantity(task.id, member.id)

    let realAmount = amount

    if (amount < 0 && currentMember <= 0) return

    if (amount < 0 && currentMember + amount < 0) {
      realAmount = -currentMember
    }

    if (amount > 0 && target > 0 && currentTotal >= target) return

    if (amount > 0 && target > 0 && currentTotal + amount > target) {
      realAmount = target - currentTotal
    }

    if (realAmount === 0) return

    const nextTotal = Math.max(0, currentTotal + realAmount)
    const now = new Date().toISOString()
    ...
    if (nextTotal === 0) {
    ...
    } else if (target > 0 && nextTotal >= target) {
      ...
      update.production_started_at = task.production_started_at || now
    }
    ...
    // Se a pessoa ainda não tinha sessão ativa, inicia o cronômetro dela.
    if (realAmount > 0 && !memberIsWorking(task.id, member.id)) {
  ...
  async function setMemberQuantity(task, member) {
    const current = memberQuantity(task.id, member.id)
    ...
    const taskCurrent = Number(task.quantity_done || 0)
    const target = Number(task.quantity_target || task.goal || 0)

    let realDifference = difference

    if (difference > 0 && target > 0 && taskCurrent + difference > target) {
      realDifference = target - taskCurrent
    }

    const nextTotal = Math.max(0, taskCurrent + realDifference)
    const now = new Date().toISOString()

    const completed = target > 0 && nextTotal >= target
    ...
    const { error: taskError } = await db
      ...
      .update({
        ...
        production_finished_at: completed ? now : null
      })
      .eq('id', task.id)
    ...
    if (realDifference > 0 && !memberIsWorking(task.id, member.id)) {
      await db.from('production_sessions').insert({
        company_id: company.id,
        task_id: task.id,
        member_id: member.id
    ...
    if (completed) await finishSessions(task.id)
  ...
  async function completeTask(task) {
    ...
    await finishSessions(task.id)
    await addEvent(task.id, null, 'task_completed', 'Tarefa concluída')
    await refreshProduction()
  }
  ...
  async function addPerson(task) {
    ...
    const available = members.filter(
      m =>
        m.active !== false &&
        !current.some(c => c.id === m.id)
    )

    if (!available.length) {
      alert('Todos os funcionários ativos já estão nesta tarefa.')
      return
    }
    ...
    const member = available[Number(answer) - 1]
    ...
    if (existing) {
      ...
      const { error } = await db.from('task_members').insert({
        task_id: task.id,
        member_id: member.id,
        active: true
      })
  ...
  function printEmployeeDay(member) {
    ...
    const rows = list.map((t, i) => `
      <div style="padding:14px 0;border-bottom:1px solid #aaa">
        ...
        Meta da tarefa: ${safe(String(t.quantity_target || t.goal || '—'))}<br>
  ...
  const producingMemberIds = [
    ...new Set(producingSessions.map(s => s.member_id))
  ]
  ...
  const performance = useMemo(() => {
    return activeMembers
      .map(member => {
        const quantity = logs
          .filter(l => {
            if (l.member_id !== member.id) return false

            const task = tasks.find(t => t.id === l.task_id)
            return task?.schedule_date === viewDate
          })
          .reduce((sum, l) => sum + Number(l.quantity || 0), 0)
  ...
  const taskProps = task => ({
    ...
    memberIsWorking,
    onStart: startTask,
    onMemberProgress: changeMemberProgress,
    onMemberQuantity: setMemberQuantity,
      ...
      <main className="page">
        {tab === 'dashboard' && (
          <>
            ...
            <section className="panel" style={{ marginTop: 20 }}>
              ...
              {dateTasks.map(task => (
                <ProductionTask key={task.id} {...taskProps(task)} />
              ))}
            </section>
          </>
        )}
        ...
        {tab === 'programacao' && (
          <>
            ...
            <section className="panel" style={{ marginTop: 20 }}>
              {filteredTasks.length === 0 && <p>Nenhuma tarefa.</p>}
    ...
    >
      <div>
        <div style={{
          ...
          <b style={{ fontSize: 20 }}>{task.title}</b>

          {working && (
            <span>🟢 {sessions.length} produzindo agora</span>
          )}
        ...
        <p>
          🏭 {sector} · Prioridade: {priorityName(task.priority)}
        </p>

        {task.notes && <p>📝 {task.notes}</p>}

        {task.deadline_time && (
          <p>⏰ Prazo: {task.deadline_time.slice(0, 5)}</p>
        )}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <strong>TOTAL DA EQUIPE</strong>
            <strong>
              {done} / {target || '—'} {target ? `— ${percent}%` : ''}
            </strong>
          </div>

          {target > 0 && <Progress percent={percent} />}
        </div>

        <h3 style={{ marginTop: 20 }}>👥 Produção individual</h3>
        ...
        <div style={{
          ...
          {people.map(person => {
            const individual = memberQuantity(task.id, person.id)
            const isWorking = memberIsWorking(task.id, person.id)
            ...
            return (
              ...
              >
                <div>
                  <b style={{ fontSize: 17 }}>
                    {isWorking ? '🟢' : '⚪'} {person.name}
                  </b>

                  <div style={{ marginTop: 5 }}>
                    Produziu: <strong>{individual}</strong>
                  </div>
                </div>
                  ...
                  {task.status !== 'completed' && (
                    <>
                      <button
                        className="secondary"
                        onClick={() =>
                          onMemberProgress(task, person, -5)
                        }
                      >
                        −5
                      </button>
        ...
        {people.map(person => (
          ...
          >
            ...
            <div style={{ fontSize: 22, marginTop: 5 }}>
              {memberQuantity(task.id, person.id)}
            </div>
          </div>
        ))}
      </div>
